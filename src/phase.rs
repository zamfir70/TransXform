...
impl PhaseController {
    ...
    fn thresholds_for(&self, phase: Phase) -> HashMap<String, f64> {
    ...
    pub fn update(
        ...
        if self.is_phase_expired(step) {
            if self.consecutive_satisfied > 0 {
                // Some progress — try to advance
                if let Some(next) = self.current.next() {
                    return Some(self.transition_to(
                        next,
                        step,
            ...
            // Duration exceeded but invariants still failing — log but don't force
        }
...
"#,
    ...
    fn advances_after_clean_steps() {
        ...
        for step in 0..10 {
            ctrl.update(&empty_violations, &empty_counts, 3, step);
        }
    ...
    fn violations_reset_counter() {
        ...
        ctrl.update(&hard_violation, &empty_counts, 3, 9);
        assert_eq!(ctrl.current_phase(), Phase::Bootstrap);
        ...
        ctrl.update(&empty_violations, &empty_counts, 3, 19);
        assert_eq!(ctrl.current_phase(), Phase::RepresentationFormation);
    }
    ...
    fn regression_on_budget_exhaustion() {
        ...
        for step in 0..10 {
            ctrl.update(&empty_violations, &HashMap::new(), 3, step);
        }
        assert_eq!(ctrl.current_phase(), Phase::RepresentationFormation);
        ...
        ctrl.update(&hard_violation, &counts, 3, 10);
        assert_eq!(ctrl.current_phase(), Phase::Bootstrap);
    }
    ...
    fn abort_on_double_regression() {
        ...
        // Advance to RepresentationFormation
        for step in 0..10 {
            ctrl.update(&empty, &HashMap::new(), 3, step);
        }

        // First regression
        let mut counts = HashMap::new();
        counts.insert("head".to_string(), 3u32);
        ctrl.update(&empty, &counts, 3, 10);
        assert_eq!(ctrl.current_phase(), Phase::Bootstrap);
    ...
    fn aborted_is_terminal() {
        ...
        let result = ctrl.update(&[], &HashMap::new(), 3, 1);
    ...
    fn allowed_interventions_by_phase() {
        ...
        for step in 0..60 {
            ctrl.update(&[], &HashMap::new(), 3, step);
        }
